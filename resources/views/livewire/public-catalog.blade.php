    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="relative bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-700 rounded-3xl p-8 md:p-12 mb-12 overflow-hidden shadow-xl border border-indigo-50/10">
            ...
            <div class="relative z-10 max-w-2xl">
                <span class="bg-indigo-500/30 backdrop-blur-sm border border-indigo-400/30 text-white text-xs px-3 py-1 rounded-full font-bold uppercase tracking-wider">Cursos de Éxito</span>
                <h1 class="text-4xl md:text-5xl font-extrabold text-white mt-4 mb-3 tracking-tight">Catálogo de Cursos</h1>
                <p class="text-indigo-100 text-base md:text-lg">Explora nuestra amplia oferta formativa y lleva tus habilidades al siguiente nivel con el mejor soporte docente.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-3">
                            <span class="bg-indigo-50 text-indigo-700 text-xs px-2.5 py-1 rounded-lg font-semibold tracking-wide uppercase">
                                {{ $course->is_classroom ? 'Presencial / Apoyo' : 'Online' }}
                            </span>
                        ...
                        <div class="mb-4 bg-indigo-50/40 border border-indigo-100/50 rounded-xl p-3 text-xs flex justify-between items-center select-none">
                            <div>
                                <p class="text-indigo-900 font-extrabold uppercase tracking-wider">🗓 Horario presencial</p>
                                <p class="text-indigo-600 font-bold mt-0.5">{{ $course->schedule }}</p>
                            </div>
                            @if($course->classroom_pass_code)
                            <div class="text-right">
                                <p class="text-gray-400 uppercase font-bold">🎫 Pase</p>
                        ...
                        <div class="flex justify-between items-center pt-4 border-t border-gray-50 mt-auto">
                            <div>
                                <p class="text-xs text-gray-400 uppercase tracking-wider font-semibold">Inversión</p>
                                <span class="text-indigo-600 font-extrabold text-2xl">${{ number_format($course->price, 2) }}</span>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-gray-400 uppercase tracking-wider font-semibold">Instructor</p>
                                <span class="text-sm text-gray-800 font-bold">{{ $course->teacher->user->name ?? 'Profesor' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-span-3 text-center py-16 bg-white rounded-3xl border border-dashed border-gray-200 shadow-sm">
                ...
                <p class="text-gray-500 text-xl font-bold">Actualmente no hay cursos publicados.</p>
